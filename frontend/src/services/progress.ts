...
import { fetchLevels, fetchWords } from './api';
...
const FALLBACK_WORDS: Record<number, string[]> = {
  1: ["sad","dad","had","ash","lad","lag","gal","jag","fad","flag","glad","flash","dash","gash","lash","hash","fall","hall","shall","glass","slash","salad","flask","half","lass","saga","alfa","as"],
  2: ["the","that","this","with","they","what","just","like","fish","wish","right","light","type","their","where","quite","quote","while","write","white","fruit","tulip","quilt","split","ought","there"],
  3: ["jump","mix","zero","box","verb","exam","next","zone","back","maze","bold","king","zoom","flex","cozy","buzz","jinx","blank","clown","cabin","vivid","waltz","rhyme","frozen","complex"],
  4: ["Hello","World","Quick","Brown","Jumps","River","Lake","Cloud","Storm","Dragon","Castle","Knight","Shield","Forest","Magic","Power","Quest","Brave","Light","Dream","Spark"],
};
...
export async function getWordsForLevel(level: LevelConfig): Promise<string[]> {
  try {
    const words = await fetchWords(level.tier);
    return words.map(w => w.text);
  } catch {
    return FALLBACK_WORDS[level.tier] || FALLBACK_WORDS[1];
  }
}

export function isLevelUnlocked(levelId: number, levels: LevelConfig[]): boolean {
  if (levelId === 1) return true;
  const level = levels.find(l => l.id === levelId);
  if (!level) return false;
  const playerData = loadPlayerData();
  const prevScore = playerData.levelScores[levelId - 1];
  return prevScore !== undefined && prevScore.stars >= level.unlockRequirement;
}

export function getTotalStars(): number {
  return loadPlayerData().totalStars;
}
